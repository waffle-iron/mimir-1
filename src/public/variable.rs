...
impl Var {
    ...
    pub fn add_ref(&self) -> Result<()> {
        try_dpi!(externs::dpiVar_addRef(self.var),
                 Ok(()),
    ...
    pub fn copy_data(&self, src_pos: u32, dst: &mut Var, dst_pos: u32) -> Result<()> {
        try_dpi!(externs::dpiVar_copyData(dst.var(), dst_pos, self.var, src_pos),
                 Ok(()),
    ...
    pub fn get_data(&self) -> Result<u32> {
        ...
        try_dpi!(externs::dpiVar_getData(self.var, &mut num_elements, &mut data_arr_ptr),
                 Ok(num_elements),
    ...
    pub fn get_num_elements(&self) -> Result<u32> {
        let mut num_elements = 0;
        try_dpi!(externs::dpiVar_getNumElementsInArray(self.var, &mut num_elements),
                 Ok(num_elements),
    ...
    pub fn get_size_in_bytes(&self) -> Result<u32> {
        let mut size = 0;
        try_dpi!(externs::dpiVar_getSizeInBytes(self.var, &mut size),
                 Ok(size),
    ...
    pub fn release(&self) -> Result<()> {
        try_dpi!(externs::dpiVar_release(self.var),
                 Ok(()),
